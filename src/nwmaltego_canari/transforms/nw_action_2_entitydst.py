...
@configure(
    label='From Action to Destination [Netwitness]',
    description='Returns Destination Entity associated with Action from Netwitness.',
    uuids=[ 'netwitness.v2.NetwitnessActionToEntityDst_Netwitness' ],
...
def dotransform(request, response, config):
    ...
    if 'ip' in request.fields:
        ...
        query = 'select ip.dst where (time=%s) && service=%s && (ip.dst=%s || ip.src=%s)' % (diff, request.fields['service'],request.fields['ip'], request.fields['ip'])
        ...
        diff = nwmodule.nwtime(config['netwitness/days'])
        query = 'select ip.dst where (time=%s) && service=%s && alias.host=%s' % (diff, request.fields['service'], request.fields['hostname'])
        json_data = json.loads(nwmodule.nwQuery(0, 0, query, 'application/json', 2500))
        entity_list = []
