...
@configure(
    label='Hostname Alias To IP Source [Netwitness]',
    description='Returns IP source addresses associated with the alias.host from Netwitness.',
    uuids=[ 'netwitness.v2.NetwitnessHostnameToIPsrc_Netwitness' ],
...
def dotransform(request, response, config):
    ...
    query = 'select ip.src where (time=%s) && alias.host=%s' % (diff, ip_entity)
