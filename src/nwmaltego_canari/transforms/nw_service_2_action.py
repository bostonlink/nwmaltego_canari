...
@configure(
    label='From Service to Action [Netwitness]',
    description='Returns Actions associated with Service from Netwitness.',
    uuids=[ 'netwitness.v2.NetwitnessServiceToAction_Netwitness' ],
...
def dotransform(request, response, config):
    ...
    if 'ip' in request.fields:
        ...
        query = 'select action where (time=%s) && service=%s && (ip.src=%s || ip.dst=%s)' % (diff, service, ip, ip)
    else:
        breadhost = request.fields['hostname']
        query = 'select action where (time=%s) && service=%s && alias.host=%s' % (diff, service, breadhost)
